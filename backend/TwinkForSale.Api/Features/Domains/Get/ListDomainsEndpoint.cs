...
public sealed class ListDomainsEndpoint(AppDbContext dbContext) : EndpointWithoutRequest<ListDomainsResponse>
{
    ...
    {
        Get("/api/admin/domains", "/api/domains");
    ...
    public override async Task HandleAsync(CancellationToken ct)
    {
        var domains = await dbContext.UploadDomains.AsNoTracking()
            ...
            .Select(x => new DomainDto(x.Id, x.Domain, x.Name, x.IsActive, x.IsDefault, x.SupportsSubdomains))
            .ToListAsync(ct);
...
public sealed record DomainDto(string Id, string Domain, string Name, bool IsActive, bool IsDefault, bool SupportsSubdomains);
