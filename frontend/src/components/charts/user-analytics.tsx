...
export const UserAnalytics = component$<UserAnalyticsProps>(
  ({ userName }) => {
    // Per-user analytics for admins is not exposed by the backend yet, so the
    // expandable panel renders the empty state until an endpoint exists.
    const analyticsData = useSignal<DailyMetric[]>([]);
