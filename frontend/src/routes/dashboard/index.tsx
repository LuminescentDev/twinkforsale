...
export const useUserData = routeLoader$(async (requestEvent): Promise<{
  ...
  // Fetch uploads and other data from API
  let uploads;
  try {
    uploads = await api.uploads.list({ page: 1, pageSize: 10 });
  } catch {
    uploads = { items: [], totalCount: 0 };
  }
  ...
  // Calculate stats from user settings
  const stats = {
    totalUploads: uploads.totalCount,
    totalViews: 0, // Would need a separate analytics endpoint
    storageUsed: user.settings?.storageUsed || 0,
    maxStorage: user.settings?.maxStorageLimit || 10737418240, // 10GB default
  };

  // Mock analytics data for now - would need a user analytics endpoint
