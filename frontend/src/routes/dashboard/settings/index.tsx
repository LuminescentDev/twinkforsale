...
import { settings as settingsApi, account as accountApi } from "~/lib/api/client";
...
export const useUploadDomainsLoader = routeLoader$(async (requestEvent) => {
  ...
  try {
    ...
    return domains.map((d) => ({
      ...
      supportsSubdomains: d.supportsSubdomains,
    }));
  } catch (error) {
  ...
  async (values, requestEvent) => {
    ...
    try {
      await settingsApi.update({
        uploadDomainId: values.uploadDomainId || undefined,
        customSubdomain: values.customSubdomain || undefined,
        defaultExpirationDays: values.defaultExpirationDays || undefined,
        defaultMaxViews: values.defaultMaxViews || undefined,
      });

      return { success: true, message: "Settings updated successfully" };
    } catch (error) {
      console.error("Failed to update settings:", error);
      return requestEvent.fail(500, { message: "Failed to update settings" });
    }
  },
  zod$({
...
export const useUpdateParticleConfigAction = routeAction$(
  async (values, requestEvent) => {
    ...
    // Particle config is stored locally for now
    // TODO: Add particle config to backend settings
...
export const useDeleteAccountAction = routeAction$(
  async (values, requestEvent) => {
    ...
    try {
      await accountApi.delete(values.confirmationText);

      // Redirect to home page after successful deletion
      throw requestEvent.redirect(302, "/");
    } catch (error: any) {
      // Check if it's a redirect (successful deletion)
      if (error?.status === 302 || error?.type === "redirect") {
        throw error;
      }
...
export default component$(() => {
  ...
  // Initialize particle config from localStorage or default
  ...
  // Sync particle settings on page load
  // eslint-disable-next-line qwik/no-use-visible-task
  useVisibleTask$(async () => {
    // Update global particle store with saved settings
