...
export const useUserData = routeLoader$(async (requestEvent) => {
  ...
  try {
    ...
    const [usersResponse, analyticsData] = await Promise.all([
      api.admin.users.list({ pageSize: 1000 }),
      api.admin.analytics(),
    ]);

    // Map users to expected format
    const users = usersResponse.items.map((u) => ({
      id: u.id,
      name: u.name,
      email: u.email,
      image: u.image,
    ...
    return {
      ...
      analyticsData: analyticsData.recentStats || [],
    };
  } catch (error) {
  ...
  try {
    ...
    if (typeof data.maxStorageLimit === "string") {
      if (data.maxStorageLimit.trim() === "") {
        updateData.maxStorageLimit = null;
      } else {
    ...
    await adminApi.users.update(data.userId as string, updateData);
    return { success: true };
  } catch (error) {
